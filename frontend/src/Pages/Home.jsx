...
import { Package, ShoppingCart, Users, Store, MapPin, X, AlertCircle, Search } from "lucide-react";
...
const Home = () => {
  ...
  const handleSell = async () => {
    ...
    if (qty <= 0) return toast.error("Quantity must be at least 1", "error");
    if (selectedProduct.inventory < qty) return toast.error("Not enough inventory", "error");

    const unitPrice = city === "johrabad" ? selectedProduct.price?.johrabad ?? 0 : selectedProduct.price?.other ?? 0;
    const totalPrice = unitPrice * qty;

    if (saleType === "full") {
      if (!customerName) return toast.error("Provide customer name", "warning");
      showConfirmDialog(
        "Confirm Full Payment Sale",
        `Sell ${qty} unit(s) for Rs. ${totalPrice.toLocaleString()} (${city}) to ${customerName}?`,
        async () => {
          ...
          if (res.success) {
            ...
            toast.success("Sold successfully!", "success");
            closeModal();
          } else toast.success(res.message || "Error recording sale", "error");
        }
      );
    } else {
      if (!customerName || paidAmount === "") return toast.success("Provide customer name and paid amount", "warning");
      const paid = Number(paidAmount);
      if (paid > totalPrice) return toast.success(`Paid cannot exceed Rs. ${totalPrice.toLocaleString()}`, "error");
      ...
      showConfirmDialog(
        ...
        async () => {
          ...
          if (res.success) {
            await fetchSoldItems();
            fetchDebts();
            toast.success("Partial sale recorded!", "success");
            closeModal();
          } else toast.error(res.message || "Error recording sale", "error");
        }
      );
    }
  };
      ...
      >
        <Fade in={isModalOpen}>
          <Box sx={modalStyle}>
            ...
            {/* Customer & Shop Fields (Shown for both full and partial) */}
            <Fade in>
              <div>
                <TextField
                  label="Customer Name"
